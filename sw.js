...
const CACHE_NAME = 'bogey-v32'; // bumped 2026-08-17 — Bar & Grill Menu heading back to "Bar & Grill" (js/app.js renderBarMenu). The "Mt. Paul" that was folded into it came from a preview where the topbar logo was a text placeholder; the real topbar carries the Mt. Paul vector logo directly above, so the words were a duplicate of it. index.html cache-buster ?devcb50 -> ?devcb51. Previously v31, 2026-08-17 — Bar & Grill Menu: Salsa restored to Add Extras (js/bar-menu.js). The 2022 menu PDF shows Salsa in the slot the current sheet fills with a second Onions row, so the earlier removal dropped a real item. Price $2.25 is INFERRED from the 2022 $2.00 plus the $0.25 uplift every other row in that block took — unconfirmed, tracked in PROJECT.md. index.html cache-buster ?devcb49 -> ?devcb50. Previously v30, 2026-08-17 — Bar & Grill Menu brand-name pass (js/bar-menu.js), Paul's call: Heyall -> Hey Y'all, Bud/Coors Lite -> Light, Millers -> Miller, Sleemans -> Sleeman (x2), Motts -> Mott's, Kahlua -> Kahlúa, jalapenos -> jalapeños. "Nudes" left as printed, unconfirmed. index.html cache-buster ?devcb48 -> ?devcb49. Previously v29, 2026-08-17 — Bar & Grill Menu: "Triple Bogie" corrected to "Triple Bogey" (js/bar-menu.js), Paul's call. index.html cache-buster ?devcb47 -> ?devcb48. Previously v28, 2026-08-17 — Bar & Grill Menu: "Warmed Liqueres" corrected to "Warmed Liqueurs" (js/bar-menu.js), Paul's call; the printed sheet has the misspelling. index.html cache-buster ?devcb46 -> ?devcb47. Previously v27, 2026-08-17 — Bar & Grill Menu: duplicate "Onions $2.25" row removed from Add Extras (js/bar-menu.js), Paul's call. index.html cache-buster ?devcb45 -> ?devcb46. Previously v26, 2026-08-17 — Bar & Grill Menu: Add Extras and Add Extras or Substitutions flattened from the two-across grid to one row per item, name and price (js/bar-menu.js). Coolers & Ciders still uses the pairs grid. index.html cache-buster ?devcb44 -> ?devcb45. Previously v25, 2026-08-17 — Bar & Grill Menu prices optically matched to the item names (css/styles.css): .bgm-price 14px -> 13px, .bgm-pair-price 13px -> 12px. Spline Sans Mono's lining digits are 0.750em against Hanken Grotesk's 0.703em cap height, so equal nominal sizes render the price ~7% taller than the name. index.html cache-buster ?devcb43 -> ?devcb44. Previously v24, 2026-08-17 — Bar & Grill Menu type scaled down one step to stop long names wrapping on narrow viewports (css/styles.css): .bgm-name/.bgm-price 15px -> 14px, .bgm-desc 13px -> 12.5px, .bgm-pair-name/.bgm-pair-price 14px -> 13px. index.html cache-buster ?devcb42 -> ?devcb43. Previously v23, 2026-08-17 — Bar & Grill Menu tightening pass (Paul): heading is now "Mt. Paul Bar & Grill" and the "Mt. Paul Golf Course" dateline is gone (js/app.js renderBarMenu); Add Extras and Add Extras or Substitutions take the sub-heading type via a new smallTitle group flag (js/bar-menu.js, css/styles.css .bgm-title-sm); Dips & Salad Dressings dropped from the two-across grid to a single name/price column with Feta Cheese and Guacamole on their own rows. index.html cache-buster ?devcb41 -> ?devcb42. Previously v22, 2026-08-17 — new Bar & Grill Menu screen: js/bar-menu.js (clubhouse menu data + renderer) added and precached, renderBarMenu() + 'barmenu' route in js/app.js, a "Bar & Grill Menu" item in the slide-out flyout, and a .bgm-* block in css/styles.css. index.html cache-buster ?devcb40 -> ?devcb41. Previously v21, 2026-08-10 — Last Round bar chart: "\u2264 Birdie" label reduced to "Birdie" (js/app.js todaysStatsHTML). The operator wrapped onto its own line in the 4-column grid and a lone \u2264 at 14px read as an "s"; the bucket (h.score < h.par) is already labelled plain "Birdie" in Score Distribution and "Birdies" in the weekly charts, so the operator was inconsistent as well as unreadable. index.html cache-buster ?devcb39 -> ?devcb40. Previously v20, 2026-07-27 — "Today's Round" renamed "Last Round: <round date>" with a new roundDateLabel() helper (js/app.js). Previously v19, 2026-07-27 — Trends heading takes a footnote asterisk, caption cut to "*Unless otherwise noted." (js/app.js trendsHTML). Previously v18, 2026-07-27 — "Last 10 Rounds" renamed "Scores: Last 10 Rounds", caption trimmed to the reading direction (js/app.js lastTenHTML). Previously v17, 2026-07-27 — Monthly Scoring Trend removed from Analytics (js/app.js; builder and stats.js monthlyScoring kept, just out of the render chain). Previously v16, 2026-07-27 — "Hole Ratings" renamed "Strokes per Hole" on Analytics, both the live section and its empty state (js/app.js). Previously v15, 2026-07-27 — Score by Day of Week removed from Analytics (js/app.js; scoreByDayHTML and stats.js scoreByDay kept, just out of the render chain). Previously v14, 2026-07-27 — Score by Day of Week caption now states the values and window (js/app.js scoreByDayHTML gains a windowLabelTitle param). Previously v13, 2026-07-27 — Stats Breakdown caption cut to the bare window label (js/app.js). Previously v12, 2026-07-27 — Analytics "Scoring Breakdown" heading renamed "Stats Breakdown" (js/app.js). Previously v11, 2026-07-27 — 1 Putts and Penalty Impact sections removed from Analytics (js/app.js; both builders and their stats.js computations kept, just out of the render chain). Previously v10, 2026-07-27 — UD/Analytics accuracy pass: "1 Putt Par Saves" replaced by "1 Putts" (js/stats.js onePutts, js/app.js), UD row dropped from 20 Round Average and Scrambling line dropped from the Putting card so UD renders once (Trends grid only), Trends UD tile repointed at the new last-20 js/stats.js scrambling20. Previously v9, 2026-07-26 — large pass: 19th Hole credit sequence added (js/app.js renderSaved/playSavedSequence/creditsHTML, css/styles.css .saved-*/.intro-*/.credit-*/.quiet-sign); Widow pairing rule corrected in js/round-record.js (resolvePendingNine no longer requires complementary halves, pairNineHoleRecords renumbers the second widow 10-18); back-nine and standalone-nine dead paths deleted from js/app.js; boot() gains reconcileStaleRound + last-screen restore (js/storage.js LAST_SCREEN key); Save now lands on the terminal 19th Hole screen instead of silently starting a new round; .btn.secondary removed; Settings and Analytics layout pass. index.html cache-buster at ?devcb39
